...
class Renderer:
    def __init__(self, width, height, bank_size, bank_count, draw_count):
        ...
        self.dc_shader = GLShader(
            DRAWING_COMMAND_VERTEX_SHADER, DRAWING_COMMAND_FRAGMENT_SHADER
        )
        self.dc_att = GLAttribute(
            DRAWING_COMMAND_ATTRIBUTE_INFO, draw_count, integer=True, dynamic=True
        )
        self.dc_tex_list = [bank._tex for bank in self.bank_list]

        self.fb_shader = GLShader(
            FRAMEBUFFER_VERTEX_SHADER, FRAMEBUFFER_FRAGMENT_SHADER
        )
    ...
    def _next_dc_data(self):
        ...
        data[CLIP_PAL_INDEX : CLIP_PAL_INDEX + CLIP_PAL_COUNT] = self.clip_pal_data
